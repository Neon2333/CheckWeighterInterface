...
namespace CheckWeighterInterface.DataAnalysis
{
    public partial class FrequencyDomainAnalysis : DevExpress.XtraEditors.XtraUserControl
    {
        ...
        public FrequencyDomainAnalysis()
        {
            ...
            SystemConfig.SystemConfig.brandChangedReInitFrequencyDomainAnalysis += reInitFrequencyDomainAnalysis;
        ...
        private void pieDataAndPointDataProcessFromStatusHistory()
        {
            ...
            for(int i = 0; i < dtStatusHistory.Rows.Count; i++)
            {
                ...
                //PointHistory数据
                int weightGram = weightIntervalProcess(Convert.ToDouble(dtStatusHistory.Rows[i]["historyWeight"]));

                if (weightAndCountGramDtPoint.ContainsKey(weightGram) == false)
                {
                    weightAndCountGramDtPoint.Add(weightGram, 1);
                }
                else
                {
                    weightAndCountGramDtPoint[weightGram]++;
                }
